...
import {
  SnakeAndLadderGame,
  type SnakeAndLadderGameProps,
} from "./Components/SnakeAndLadderGame";
...
function App() {
  ...
  // max players-4, max snaked/ladders-10
      ...
      {!gameData ? (
        <div className="relative  w-3/4 h-3/4 rounded-4xl border-solid border-2">
          <form onSubmit={handleSubmit}>
            ...
            <button className="rounded-full border-5 border-black/60 hover:border-black hover:scale-[1.03] hover:shadow-lg hover:bg-gray-100 px-3 py-2 transition-all duration-100 absolute bottom-5 left-1/2 translate-x-[-50%]">
              Start Game
            </button>
          </form>
        </div>
