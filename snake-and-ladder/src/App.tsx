...
import { gameTitle } from "./utils/Utils";
...
function App() {
  ...
  return (
    <div className="relative bg-emerald-50 w-screen h-screen items-center justify-center flex flex-col">
      {!gameData ? (
        <>
          <div className="game-title">{gameTitle}</div>
          <div className="relative  w-3/4 flex-1 mb-10 rounded-4xl border-solid border-2">
            <form onSubmit={handleSubmit}>
              <AddInput
                title="Select number of Snakes(max-15):"
                type="number"
                placeholder="10"
                min={0}
                max={15}
